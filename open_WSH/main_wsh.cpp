...
void write_report(ofstream& outFile,
                  double sn,
                  int errbit_cnt,
                  int block_err,
                  long i,
    ...
    outFile << "now is " << 1900 + ltm->tm_year << "/" << 1 + ltm->tm_mon
            << "/";
    outFile << ltm->tm_mday << " " << ltm->tm_hour << ":" << ltm->tm_min << ":"
            << ltm->tm_sec << endl;
    ...
    outFile << "Average_Error = " << avg_hard_error << " ; // " << binary_flip
            << "/65535";
    outFile << endl;
    ...
    if (block_err > 0)
        outFile << "BLER          = " << (block_err_rate) << " ; "
                << 10 * log10(block_err_rate) << "dB" << endl;
    else
        outFile << "BLER = " << (block_err_rate) << " ;" << endl;

    // outFile << "out_BER = " << out_BER << " ;" << endl;
    outFile << "  ----  Correctable initial Error Count ----- " << endl;
    for (j = 0; j < max_hist; j++) {
        if (cnt_cor[j] != 0)
            outFile << "Correctable_Error_" << j + 1 << " = " << cnt_cor[j]
                    << " ;" << endl;
    }
    outFile << "  ----  Uncorrectable initial Error Count --- " << endl;
    for (j = 0; j < max_hist; j++) {
        if (cnt_uncor[j] != 0)
            outFile << "Uncorrectable_Error_" << j + 1 << " = " << cnt_uncor[j]
                    << " ;" << endl;
    }
    outFile << "------------------------------------------------------- "
            << endl;
    if (flag == 1) {
        ...
        cout << "BLER          = " << (block_err_rate) << " = " << block_err
             << "/" << i << ", " << errbit_cnt << endl;
        // cout << "out_BER       = " << out_BER << " ;" << endl;
    }
}

void bpsk_awgn(__m256i m256z[],
               long* idum,
               double var,
               int* tx_code,
               char* lratio,
    ...
    const char ch_idx = 4;
    //
    //  change ch_idx to 7 when hard decoding, but need change decoder parameter
    //  for better quality
    //
    ...
    for (i = 0; i < var_N / 2; i++) {
        ...
        sign = ((a1 + n1) >= 0) ? 1 : -1;
        if (((a1 + n1) > 0.5) || ((a1 + n1) < -0.5))
            lratio[2 * i] =
                sign * 7;  // 7 is channel-bit-index, not llr for calculation
        else
            lratio[2 * i] =
                sign *
                ch_idx;  // 4 is channel-bit-index, not llr for calculation

        sign = ((a2 + n2) >= 0) ? 1 : -1;
        if (((a2 + n2) > 0.5) || ((a2 + n2) < -0.5))
            lratio[2 * i + 1] =
                sign * 7;  // 7 is channel-bit-index, not llr for calculation
        else
            lratio[2 * i + 1] =
        ...
        if ((lratio[2 * i] < 0) ^ (tx_code[2 * i] == 1))
            num_awgn++;
        if ((lratio[2 * i + 1] < 0) ^ (tx_code[2 * i + 1] == 1))
            num_awgn++;
    }
...
void* job_thread(void* args) {
    ...
    for (ii = 0;
         (ii < (test_per_msg * (1 + 1))) && (input->block_err < min_error);
         ii++) {
        ...
        if (ans_wrong == 1) {
            ...
            if (this_err >= max_hist)
                cnt_alias[max_hist - 1]++;
            else if (this_err > 0)
                cnt_alias[this_err - 1]++;
        }

        if (dec_ok == 0) {
            if (longi > test_per_msg)
                cout << num_awgn << ", " << input->block_err << '/' << ii
                     << endl;
        ...
        if (ii == (test_per_msg * (idx + 1) - 1))
            cout << input->errbit_cnt << ':' << input->block_err << ':' << ii
                 << endl;
    }
    input->block_i = ii;
...
int main(void) {
    ...
    *idum = -83168;  // rand_seed
    ...
    cw_bit = (unsigned long long*)malloc(sizeof(unsigned long long) *
                                         (int)(var_N / 64));
    ...
    //       so data  bits start from cw_bit[ check_M/64 ] to cw_bit[ var_N/64
    //       -1 ]
    //
    ...
    for (sn = sn_start; sn <= sn_end; sn = sn + sn_step) {
        noise_var2 =
            0.5 * pow(10, (sn / -10)) *
            (static_cast<double>(var_N) / static_cast<double>(var_N - check_M));
        var = sqrt(noise_var2);
        ...
        while ((longi < min_loop) || (block_err < error_limit)) {
            rc = pthread_create(&threads[0], NULL, job_thread,
                                (void*)&(pd_in[0]));
            if (rc) {
            ...
            if ((longi > snr_runs))
                break;
            if (longi >= test_per_msg)
                cout << errbit_cnt << ':' << block_err << ':' << longi << endl;
    ...
    for (ii = 0; ii < 20; ii++) {
        ...
        if (dec_ok == 1) {
            for (j = 0, k = 0; j < var_N; j++)
                if (dec_bit[k] != encoded_bit[k])
                    k++;

            if (k != 0)
                printf("miscorrection! %d\n", k);
            if (num_dec != num_awgn)
                printf("wrong number %d, %d!\n", num_dec, num_awgn);
        }
        printf("%d cw, num_awgn = %d, num_dec = %d, using %d cycle\n", ii + 1,
               num_awgn, num_dec, cycle);
    }
}
